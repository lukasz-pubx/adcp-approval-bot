...
3. **Otherwise, load the docs index: fetch https://docs.adcontextprotocol.org/llms.txt.**
   This is the machine-readable index of the entire AdCP documentation — every page with
   a one-line summary. It is how you discover which docs to load; do not guess page URLs
   and do not rely on memorised spec details. Fetch it with whatever web access you have:
   the **context7 MCP** for documentation lookups, the **tavily MCP** for web search, a
   direct web fetch, or any other web tool available in your environment.
4. **Select and fetch the right pages from the index.** Use your judgment: from what the
   ...
   - the **technical specification** for the protocol domain in play — the spec pages
     ...
     like *Calling an AdCP agent*, *Task Lifecycle*, and *Security*);
   ...
   Then review the implementation against what those fetched pages actually say.
5. **Use the local digest as a checklist, not a substitute.** The files under
   ...
   digest disagree, the live page wins; note the drift in your review so the digest can
   be updated. Only when no web access is available at all should you review from the
   digest alone (and say so explicitly in the review).
...
- Only raise an AdCP conformance finding if it is supported by a specific rule in the
  fetched live docs or the fallback digest. When you flag a violation, **name the rule
  you are applying and the doc page it comes from** (e.g. "Task Lifecycle: polling is
  for `submitted` only" or "digest, Lifecycle: fresh `idempotency_key` on retry creates
  a duplicate operation") so the author can verify it. Do not invent protocol
